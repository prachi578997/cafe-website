...
/* =========================================================
   ...
   MENU + BUILD YOUR COFFEE + CSV + GOOGLE SHEET
   RESERVATION QUANTITY + NO DUPLICATES
========================================================= */
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $milk =
        trim($_POST["milk"] ?? "Regular Milk");

    $sweetness =
        trim($_POST["sweetness"] ?? "Normal");

    $topping =
        trim($_POST["topping"] ?? "No Topping");
    ...
    elseif (
        !isset($coffeePrices[$coffee])
    ) {

        $orderError =
            "Please select a valid coffee.";

    }

    elseif (
        !isset($sizePrices[$size])
    ) {
    ...
    elseif (
        !isset($milkPrices[$milk])
    ) {

        $orderError =
            "Please select a valid milk.";

    }

    elseif (
        !isset($sweetnessPrices[$sweetness])
    ) {

        $orderError =
            "Please select a valid sweetness.";

    }

    elseif (
        !isset($toppingPrices[$topping])
    ) {

        $orderError =
            "Please select a valid topping.";
...
$menu = [
    ...
    "Coffee" => [
        ...
        [
            "Flavoured Coffee",
            180,
            "flavoured-coffee.jpg"
        ],

        [
            "Signature Coffee",
            220,
            "signature-coffee.jpg"
        ]
...
<section class="menu-section">
    ...
    <?php foreach($menu as $category => $items): ?>
        ...
        <div
            class="menu-category"
            data-category="<?php
                echo htmlspecialchars($categoryClass);
            ?>"
        >
    ...
    >
        ...
        <?php if($orderError !== ""): ?>
            ...
            <div class="error">
                ...
                ❌
                <?php
                    echo htmlspecialchars($orderError);
                ?>
